...
pub const red_sphere_no_shading = struct {
    ...
    pub const SphereRayIntersection = enum { secant, tangent };
    /// `∀ vectors v, v² ≔ v·v`
    ///
    /// Sphere equation
    /// ```text
    ...
    pub fn sphereRayIntersection(sphere: Sphere, ray: Ray) ?SphereRayIntersection {
    ...
    pub fn rayColor(ray: Ray, sphere: Sphere) Rgb(u8) {
        return if (sphereRayIntersection(sphere, ray)) |_|
            colors(u8).red
        else
...
pub const red_sphere_surface_normals = struct {
    pub const default = struct {
        pub const path = default_path(@This());
        pub const header = blue_gradient.default.header;
        pub const camera_center = blue_gradient.default.camera_center;
        ...
        pub const pixel_delta_y = blue_gradient.default.pixel_delta_y;
        pub const top_left_pixel_center = blue_gradient.default.top_left_pixel_center;
        pub const sphere = red_sphere_no_shading.default.sphere;
    };
    pub const Sphere = red_sphere_no_shading.Sphere;
    pub const oldSphereRayIntersection = red_sphere_no_shading.sphereRayIntersection;
    pub const SphereRayIntersection = red_sphere_no_shading.SphereRayIntersection;
    /// `(-b ± √(4·a·c)) / 2·a`
    pub fn quadratic_formula(a: R, b: R, c: R) ?[2]R {
        const discriminant = (b * b) - (4 * a * c);
        const no_real_solutions = discriminant < 0;
        if (no_real_solutions) return null;
    ...
    pub fn sphereRayIntersection(sphere: Sphere, ray: Ray) ?R {
        // move the center of the sphere to the origin of the ray. (C - Q)
        const offset_center = sphere.center.subtract(ray.origin);
        ...
        const solution = quadratic_formula(a, b, c) orelse return null;

        return solution[0];
    }
    pub fn rayColor(ray: Ray, sphere: Sphere) Rgb(u8) {
        if (sphereRayIntersection(sphere, ray)) |t| {
            if (t > 0) {
                const normal = ray.at(t).subtract(sphere.center).normalize() catch R3.zero;
                return normal
                    .add(R3.ones)
                    .scalarDivide(2)
                    .percentOfInteger(u8);
        ...
        return blue_gradient.rayColor(ray);
    }
    pub fn write(
        output: *Io.Writer,
        args: struct {
...
test red_sphere_surface_normals {
    try save(std.testing.io, red_sphere_surface_normals);
}
